<template>
	<div class="maps" v-if="this.transport.length !== 0">
		<gmap-map
	      ref="gmap"
	      :center="center"
	      :zoom="10"
	    ...
	    <GmapCluster @click="toggleClusterInfo">
			<gmap-marker
		        :key="index"
		        v-for="(m, index) in this.transport"
		        :title="m.id.toString()"
		        :position="{
    ...
    methods: {
		toggleClusterInfo(cluster) {
		    cluster.markers_.forEach((val,idx) => {
		    	let index = this.transport.findIndex(o => o.id == val.title)
		    	this.toggleInfoWindow(this.transport[index], index)
		    })
		 },
		getCoords(lat,lng) {
			return {
